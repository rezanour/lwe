...
VulkanGraphicsDevice::VulkanGraphicsDevice(VkPhysicalDevice const device)
: device_(device) {
  vkGetPhysicalDeviceMemoryProperties(device_, &memory_properties_);

  for (uint32_t mem_index = 0; mem_index < memory_properties_.memoryHeapCount; ++mem_index) {
    if (VK_MEMORY_HEAP_DEVICE_LOCAL_BIT & memory_properties_.memoryHeaps[mem_index].flags) {
      local_memory_bytes_ += memory_properties_.memoryHeaps[mem_index].size;
    }
    total_memory_bytes_ += memory_properties_.memoryHeaps[mem_index].size;
  }

  vkGetPhysicalDeviceProperties(device_, &properties_);
  device_name_ = properties_.deviceName;
  switch (properties_.deviceType) {
    case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:   device_type_ = GraphicsDeviceType::Discrete;   break;
    case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU: device_type_ = GraphicsDeviceType::Integrated; break;
  }
}
...
bool VulkanGraphicsSystem::Initialize() {
  ...
  for (auto const &physical_device : physical_devices) {
    devices_.push_back(std::make_shared<VulkanGraphicsDevice>(physical_device));
    LWE_LOG(Info, "Added device: %s", devices_.rbegin()->get()->Name().c_str());
  }
...
bool VulkanGraphicsSystem::EnumerateDeviceLayers(VkPhysicalDevice const physical_device, std::vector<VkLayerProperties> &out_layers) {
  uint32_t count = 0;
...
bool VulkanGraphicsSystem::EnumerateDeviceExtensions(VkPhysicalDevice const physical_device, char const *layer_name, std::vector<VkExtensionProperties> &out_extensions) {
  uint32_t count = 0;
...
void VulkanGraphicsSystem::EnumerateDeviceQueueFamilies(VkPhysicalDevice const physical_device, std::vector<VkQueueFamilyProperties> &out_families) {
  uint32_t count = 0;
